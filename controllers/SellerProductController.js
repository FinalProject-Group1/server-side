...
class SellerProductController{
    static async sellerAdd(req, res, next){
        try {
            ...
            const seller = await SellerProduct.findOne({where: {SellerId: SellerId, ProductId: +ProductId}})
            // console.log(seller, "<< ini hasilnya apa?")
            if(seller) throw ({name: "AddError"})


            const create = await SellerProduct.create({ProductId: +ProductId, stock: +stock, price: +price, SellerId, status})
    ...
    static async sellerEdit(req, res, next){
        try {
            ...
            await sellerProduct.update({ProductId: +ProductId, stock: +stock, price: +price, SellerId, status})
